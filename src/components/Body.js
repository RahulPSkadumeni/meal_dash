...
const Body = () => {
  ...
  async function getRestaurants() {
    ...
    try {
      const response = await fetch(
        "https://corsproxy.io/?https://www.swiggy.com/dapi/restaurants/list/v5?lat=9.93442534&lng=76.280789&page_type=DESKTOP_WEB_LISTING"
      );
  ...
  async function cusinelist() {
    ...
    try {
      ...
      async function cusines(jsonData) {
        for (let i = 0; i < jsonData?.data?.cards.length; i++) {
          // initialize checkData for Swiggy Restaurant data
          let cusines = json?.data?.cards[i]?.card?.card?.facetList?.filter(
            (ob) => ob.label === "Cuisines"
          )[0].facetInfo;
          // console.log(cusines)
  ...
  if (!allRestaurants) {
    ...
    var filteredData = allRestaurants.filter((restaurant) => {
      ...
      const TopRatedRestaurants = check
        ? restaurant?.info?.avgRating > 4.3
        : allRestaurants;
      const categoryMatch =
        selectedCategories.length === 0
          ? "No Matched Restaurants Found"
                ...
                <>
                  <div className="checkbox-wrapper">
                    <input
                      ...
                      className="form-check-input"
                      value={cusine?.label}
                      checked={selectedCategories.includes(cusine?.label)}
                      onChange={handleCheckboxChange}
                      key={index}
                    />
                    <label>{cusine?.label}</label>
                  </div>
                </>
              );
            })}
